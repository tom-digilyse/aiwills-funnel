...
function scrapeBrand(html, css, baseUrl){
  ...
  return {
    ...
    footer_menu_json: JSON.stringify(nav),   // footer menus and nav menus usually carry the same links; leaving this blank silently inherited the PREVIOUS client's footer
    company_address: '',
    will_price: '',
    legal_footer: ''
  };
  ...
  try {
    ...
    if (req.method === 'POST' && pathOnly === '/api/checkout'){
      ...
      try {
        ...
        // mirror_price: the TOTAL for a pair of mirror wills. Optional; blank or invalid
        // falls back to charging will_price twice, which is what every firm got before.
        const mpP = pence(cv['mirror_price']);
        ...
        const useMirrorP = (willQty === 2 && Number.isInteger(mpP) && mpP >= 100);
        const amount = (useMirrorP ? mpP : wpP * willQty) + lpP * lpaQty;
        ...
        if (!Number.isInteger(amount) || amount < 100){
          console.error('checkout: bad amount for ' + loc + ' (will_price=' + JSON.stringify(cv['will_price']) + ', lpa_price=' + JSON.stringify(cv['lpa_price']) + ', mirror_price=' + JSON.stringify(cv['mirror_price']) + ', willQty=' + willQty + ', lpaQty=' + lpaQty + ')');
        ...
        const sparams = {
          ...
          'line_items[0][quantity]': isLpaOnly ? lpaQty : (useMirrorP ? 1 : willQty),
          'line_items[0][price_data][currency]': 'gbp',
          'line_items[0][price_data][unit_amount]': isLpaOnly ? lpP : (useMirrorP ? mpP : wpP),
