    ...
    function awOpenLogin(){
      ...
      document.getElementById('awloginsend').onclick=function(){
        ...
        fetch(API+'/api/edit-request',{method:'POST',body:JSON.stringify(body)}).then(function(r){return r.json();}).then(function(j){
          if(chan==='sms'){
          /* A link texted to a phone signs in the phone, not the screen the person is sitting at.
             The text carries a 6 digit code typed here instead; its link still works for someone
             who genuinely is on their phone. */
          m.innerHTML='<div style="background:#fff;max-width:420px;width:100%;border-radius:16px;padding:26px 24px;box-shadow:0 10px 40px rgba(0,0,0,.2);text-align:center">'
            + '<h2 style="font-family:var(--hf,Georgia,serif);color:var(--heading);margin:0 0 8px;font-size:21px">Enter your code</h2>'
            + '<p style="color:var(--muted);font-size:14px;line-height:1.55;margin:0 0 14px">If we have your details, a text with a 6 digit code is on its way to <strong>'+esc(v)+'</strong>. Type it below to sign in on this screen. The text also carries a link, tapping that signs your phone in instead.</p>'
            + '<input id="awcodein" inputmode="numeric" autocomplete="one-time-code" maxlength="6" placeholder="6 digit code" style="width:100%;box-sizing:border-box;padding:12px 14px;border:1px solid var(--line);border-radius:10px;font-size:19px;letter-spacing:6px;text-align:center;font-family:var(--bf)">'
            + '<div id="awcodemsg" style="font-size:13px;margin-top:10px;min-height:18px;color:var(--muted)"></div>'
            + '<button type="button" id="awcodego" style="width:100%;margin-top:6px;background:var(--btn-bg,var(--primary));color:#fff;border:none;border-radius:var(--btn-radius,10px);padding:13px;font-weight:600;cursor:pointer;font-family:var(--bf)">Sign in</button>'
            + '<button type="button" id="awloginok" style="width:100%;margin-top:6px;background:none;border:none;color:var(--muted);cursor:pointer;font-size:13px">Cancel</button>'
            + '</div>';
          try{ document.getElementById('awcodein').focus(); }catch(e){}
          var cgo=document.getElementById('awcodego');
          if(cgo) cgo.onclick=function(){
            var cm=document.getElementById('awcodemsg');
            var cv=(document.getElementById('awcodein').value||'').replace(/[^0-9]/g,'');
            if(cv.length!==6){ cm.style.color='#c8100d'; cm.textContent='Please enter the 6 digit code from the text.'; return; }
            cgo.disabled=true; cm.style.color='var(--muted)'; cm.textContent='Checking...';
            fetch(API+'/api/code-login',{method:'POST',body:JSON.stringify({locationId:loc,phone:v,code:cv,funnel:'wills'})}).then(function(r){return r.json();}).then(function(j2){
              if(j2&&j2.ok&&j2.session){ try{ awSessSet(j2.session); }catch(e){} cm.textContent='Signed in.'; location.reload(); return; }
              cgo.disabled=false; cm.style.color='#c8100d'; cm.textContent=(j2&&j2.error)||'That code did not match. Check it and try again.';
            }).catch(function(){ cgo.disabled=false; cm.style.color='#c8100d'; cm.textContent='Something went wrong, please try again.'; });
          };
          var okb=document.getElementById('awloginok'); if(okb) okb.onclick=function(){ m.parentNode&&m.parentNode.removeChild(m); };
          return;
          }
          m.innerHTML='<div style="background:#fff;max-width:420px;width:100%;border-radius:16px;padding:26px 24px;box-shadow:0 10px 40px rgba(0,0,0,.2);text-align:center">'
            + '<h2 style="font-family:var(--hf,Georgia,serif);color:var(--heading);margin:0 0 8px;font-size:21px">Check your email</h2>'
            ...
            + '<p style="color:var(--muted);font-size:13px;line-height:1.55;margin:12px 0 18px">Nothing after a couple of minutes? Check your spam folder, and check these are the same details you first signed up with. A different or mistyped address gets no email, and this screen cannot say which emails hold an account. If in doubt, close this and press <strong>Get started</strong> on any service, registering takes a minute and signs you in straight away.</p>'
...
var LPA_FUNNEL = [
  ...
  { id:'attorneys', name:'Attorneys', title:'Your attorneys', lead:'The people you appoint to make decisions. Add up to four, each with an optional replacement.', fields:[
    { key:'list', type:'repeater', itemLabel:'Attorney', required:true, max:4, emptyMsg:'Add at least one attorney.', fields:[
      ...
      { key:'address', type:'text', label:'Address line 1', required:true },
      { type:'row', fields:[ {key:'city',type:'text',label:'Town / city',required:true}, {key:'postcode',type:'text',label:'Postcode',required:true} ] },
      { type:'row', fields:[ {key:'phone',type:'tel',label:'Phone',required:true}, {key:'email',type:'email',label:'Email',required:true} ] },
      { key:'relationship', type:'select', options:attorneyRelOpts, label:'How do you know them?', required:true },
      { key:'isTrustCorp', type:'radio', label:'Is this attorney a trust corporation? (rare - usually No)', required:true, reflow:true, options:['No','Yes'] },
      ...
      { key:'hasReplacement', type:'radio', label:'Add a replacement attorney for this person?', required:true, reflow:true, options:['Yes','No'] },
      { key:'repTitle', type:'select', label:'Replacement title', required:true, options:['Mr','Mrs','Miss','Ms','Mx','Dr','Prof','Other'], showIf:function(s,b){return getP(b+'.hasReplacement')==='Yes';} },
