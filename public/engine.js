...
function fld(base,f){
  ...
  if(f.type==='file'){ var fn=getP(p)||''; var _furl=''; (window.AIWILLS_FILES||[]).forEach(function(x){ if((x.field||'').toLowerCase()===(f.field||'').toLowerCase()) _furl=x.url; }); var _cur=(fn||_furl)?('<div style="font-size:14px;margin:2px 0 6px">Current: '+esc(fn||'document')+(_furl?(' &middot; <a href="'+esc(_furl)+'" target="_blank" rel="noopener">View</a>'):'')+' &middot; <a href="#" class="frm" data-frm="'+esc(f.field||'')+'" data-namekey="'+p+'">Remove</a></div>'):''; return '<div class="field" data-f="'+p+'">'+lab+_cur+'<input type="file" data-upload="'+esc(f.field||'')+'" data-namekey="'+p+'"'+(f.accept?(' accept="'+esc(f.accept)+'"'):'')+'><div class="uplstat" style="font-size:14px;color:var(--muted);margin-top:6px">'+(fn?('Replace: '+esc(fn)):'')+'</div></div>'; }
...
function review(){
  ...
  if(window.AIWILLS_EDIT===true){ var _tok=window.AIWILLS_TOKEN||''; var _isE=(FUNNEL===ETB_FUNNEL); var _dl='<a class="btn wide" href="'+API+(_isE?'/api/etb-pdf?t=':'/api/will-pdf?t=')+encodeURIComponent(_tok)+'" target="_blank" rel="noopener" style="display:block;text-align:center;text-decoration:none;margin-top:8px">'+(_isE?'Download summary (PDF)':'Download your will (PDF)')+'</a>'; var _fl=window.AIWILLS_FILES||[]; var _fh=(_isE&&_fl.length)?('<div class="sum" style="margin-top:12px"><h3>Your documents</h3>'+_fl.map(function(f){return '<div style="padding:4px 0"><a href="'+esc(f.url)+'" target="_blank" rel="noopener">'+esc(f.field||'Document')+'</a></div>';}).join('')+'</div>'):''; return html+_dl+_fh; }
...
var _autoT; function autosave(){ if(!loc) return; clearTimeout(_autoT); _autoT=setTimeout(function(){ try{ collectVisible(); saveToGhl(state); }catch(e){} }, 1000); } // persist on every change, not just Continue
...
document.addEventListener('click', function(e){ var rm=e.target.closest&&e.target.closest('.frm'); if(!rm) return; e.preventDefault(); var field=rm.getAttribute('data-frm'), nameKey=rm.getAttribute('data-namekey'); if(nameKey) setP(nameKey,''); window.AIWILLS_FILES=(window.AIWILLS_FILES||[]).filter(function(x){ return (x.field||'').toLowerCase()!==(field||'').toLowerCase(); }); var tok=window.AIWILLS_TOKEN||''; if(tok){ try{ fetch(API+'/api/etb-file-remove',{method:'POST',body:JSON.stringify({t:tok,field:field})}).catch(function(){}); }catch(e2){} } try{ autosave(); }catch(e3){} render(); });
