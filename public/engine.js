...
function render(){
  ...
  if(s.kind==='payment'){
    ...
    setTimeout(function(){ try{ fetch(API+'/api/will-save',{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({locationId:loc,contactId:_wcid,state:state,pdf:true})}).then(function(r){return r.json();}).then(function(j){ if(j&&j.contactId) window.AIWILLS_CONTACT_ID=j.contactId; var w=el('willpdfwrap'); if(!w) return; if(j&&j.token){ var u=API+'/api/will-pdf?t='+encodeURIComponent(j.token); w.innerHTML='<iframe src="'+u+'" style="width:100%;height:560px;border:1px solid #e0e0e0;border-radius:10px;margin-top:16px;background:#fff" title="Your will"></iframe><div style="margin-top:12px"><a class="btn wide" href="'+u+'" target="_blank" rel="noopener">Download your will (PDF)</a></div>'; } else { w.innerHTML='<p class="note">Could not generate the will document'+((j&&j.error)?(': '+esc(j.error)):'')+'.</p>'; } }).catch(function(){ var w=el('willpdfwrap'); if(w) w.innerHTML='<p class="note">Could not generate the will document.</p>'; }); }catch(e){} },60);
